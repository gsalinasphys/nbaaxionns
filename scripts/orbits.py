...
def grav_en(p: object, NS: object) -> np.ndarray:
    return axionmass*NS.grav_pot(p.positions)
...
def min_approach(p: object, NS: object) -> np.ndarray:
    ...
    rmins = -G*NS.mass*axionmass/(2*energies) + np.sign(energies)*0.5*np.sqrt((G*NS.mass*axionmass/energies)**2 + 2*axionmass*ang_momenta**2/energies)
    return rmins
...
def trajs(p: object, NS: object, rprecision: float = 1e-3, rlimits: np.ndarray = None, fname: str = None) -> None:
    rmax = max(mag(p.positions))
    
    data = [[] for i in np.arange(8)]
